...
from . import frames_db_utils, scrape_utils
...
async def main(daemon: bool):
    while True:
        ...
        url_records = frames_db_utils.fetch_unparsed_urls(
            logger, pg_dsn, settings.FRAMES_BATCH_SIZE
        )
        ...
        if len(url_records) > 0:
            url_parts = [
                scrape_utils.parse_url(logger=logger, url_id=record[0], url=record[1])
        ...
        url_records = frames_db_utils.fetch_unprocessed_urls(
            logger, pg_dsn, settings.FRAMES_BATCH_SIZE
        )
        ...
        if len(url_records) > 0:
            http_timeout = aiohttp.ClientTimeout(
                sock_connect=settings.FRAMES_SCRAPE_CONNECT_TIMEOUT_SECS,
                sock_read=settings.FRAMES_SCRAPE_READ_TIMEOUT_SECS,
            )
            connector = aiohttp.TCPConnector(
                ttl_dns_cache=3000, limit=settings.FRAMES_SCRAPE_CONCURRENCY
            )
            ...
            with Timer(name="categorize_url"):
                async with http_conn_pool:
                    ...
                    url_categories = await asyncio.gather(
