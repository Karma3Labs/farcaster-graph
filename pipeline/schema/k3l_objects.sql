...
CREATE INDEX CONCURRENTLY follows_fid_target_timestamp_idx ON neynarv3.follows(fid, target_fid, timestamp)
WHERE
    deleted_at IS NULL;

CREATE INDEX CONCURRENTLY reactions_type_target_timestamp_idx ON neynarv2.reactions(reaction_type, target_fid, timestamp)
WHERE
    target_fid IS NOT NULL;

CREATE INDEX CONCURRENTLY reactions_timestamp_type_idx ON neynarv2.reactions("timestamp", reaction_type)
WHERE
    target_fid IS NOT NULL;

CREATE INDEX CONCURRENTLY reactions_target_hash_deleted_at_timestamp_idx ON neynarv2.reactions USING btree (target_hash, deleted_at, "timestamp" DESC) WITH (deduplicate_items = 'false');
...
SELECT
,
    REFERENCES ON TABLES TO k3l_readonly;
...
,
    DELETE,
    REFERENCES ON public.k3l_recent_frame_interaction TO k3l_readonly;
...
ALTER TABLE
    ONLY public.pretrust
ADD
    CONSTRAINT fid_insert_ts_unique UNIQUE (fid, insert_ts);

----------------------------------------------------------------------------------------------------------------
CREATE MATERIALIZED VIEW public.k3l_rank AS WITH latest_gt AS (
    SELECT
        max(globaltrust_1.date) AS dt,
        globaltrust_1.strategy_id
    FROM
        public.globaltrust globaltrust_1
    GROUP BY
        globaltrust_1.strategy_id
),
latest_gt_config AS (
    SELECT
        DISTINCT ON (globaltrust_config.strategy_id) globaltrust_config.strategy_id,
        globaltrust_config.strategy_name
    FROM
        public.globaltrust_config
    ORDER BY
        globaltrust_config.strategy_id,
        globaltrust_config.date DESC
)
SELECT
    row_number() OVER () AS pseudo_id,
    row_number() OVER (
        PARTITION BY globaltrust.date,
...
FROM
    (
        (
            public.globaltrust
            JOIN latest_gt_config ON (
                (
                    globaltrust.strategy_id = latest_gt_config.strategy_id
                )
            )
        )
        JOIN latest_gt ON (
            (
                (globaltrust.strategy_id = latest_gt.strategy_id)
                AND (globaltrust.date = latest_gt.dt)
            )
        )
    ) WITH NO DATA;
...
CREATE INDEX k3l_rank_strategy_id_idx ON public.k3l_rank USING btree (strategy_id) CREATE INDEX k3l_rank_profile_id_idx ON public.k3l_rank USING btree (profile_id) ------------------------------------------------------------------------------------
...
ALTER TABLE
    public.k3l_url_labels
ALTER COLUMN
    url_id
ADD
    GENERATED ALWAYS AS IDENTITY (
        SEQUENCE NAME public.k3l_url_labels_url_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
    );

ALTER TABLE
    ONLY public.k3l_url_labels
ADD
    CONSTRAINT k3l_url_labels_pkey PRIMARY KEY (url_id);

ALTER TABLE
    ONLY public.k3l_url_labels
ADD
    CONSTRAINT k3l_url_labels_url_unique UNIQUE (url);
...
------------------------------------------------------------------------------------
CREATE TABLE public.k3l_cast_embed_url_mapping (url_id bigint, cast_id bigint);
...
ALTER TABLE
    ONLY public.k3l_cast_embed_url_mapping
ADD
    CONSTRAINT k3l_cast_embed_url_mapping_cast_id_fkey FOREIGN KEY (cast_id) REFERENCES public.casts(id);

ALTER TABLE
    ONLY public.k3l_cast_embed_url_mapping
ADD
    CONSTRAINT k3l_cast_embed_url_mapping_url_id_fkey FOREIGN KEY (url_id) REFERENCES public.k3l_url_labels(url_id);
...
FROM
    (
        (
            public.casts
            JOIN public.k3l_cast_embed_url_mapping url_map ON (
                (url_map.cast_id = casts.id)
                AND (casts.deleted_at IS NULL)
            )
        )
        JOIN public.k3l_url_labels urls ON (
            (
                (urls.url_id = url_map.url_id)
                AND ((urls.category) :: text = 'frame' :: text)
            )
        )
    )
WHERE
    public.casts.timestamp BETWEEN now() - interval '30 days'
    AND now()
GROUP BY
    casts.fid,
    'cast' :: text,
...
FROM
    (
        (
            (
                public.casts
                JOIN public.reactions ON (
                    (
                        (reactions.target_hash = casts.hash)
                        AND (reactions.reaction_type = 2)
                        AND (casts.deleted_at IS NULL)
                    )
                )
            )
...
WHERE
    ...
    AND casts.deleted_at IS NULL
    AND casts.timestamp BETWEEN now() - interval '30 days'
...
SELECT
,
    REFERENCES ON public.k3l_recent_parent_casts TO k3l_readonly;
...
CREATE INDEX k3l_channel_fids_ch_strat_ts_idx ON public.k3l_channel_fids USING btree (channel_id, strategy_name, compute_ts) CREATE INDEX k3l_channel_fids_strat_ts_idx ON public.k3l_channel_fids USING btree (strategy_name, compute_ts) ------------------------------------------------------------------------------------
CREATE TYPE channel_rank_status AS ENUM (
    'pending',
    'inprogress',
    'completed',
    'errored',
...
CREATE TABLE k3l_channel_rank_log_y2025m03 PARTITION OF k3l_channel_rank_log FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_channel_rank_log_y2025m04 PARTITION OF k3l_channel_rank_log FOR
VALUES
FROM
...
CREATE INDEX k3l_cast_action_fid_idx ON public.k3l_cast_action USING btree(fid);

CREATE INDEX k3l_cast_action_fid_ts_idx ON public.k3l_cast_action USING btree(fid, action_ts);

CREATE INDEX k3l_cast_action_cast_hash_idx ON public.k3l_cast_action USING btree(cast_hash);

CREATE INDEX k3l_cast_action_timestamp_idx ON public.k3l_cast_action USING btree (action_ts);

CREATE UNIQUE INDEX k3l_cast_action_unique_idx ON public.k3l_cast_action USING btree(cast_hash, fid, action_ts);

CREATE TABLE k3l_cast_action_y2024m04 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-04-01') TO ('2024-05-01');

CREATE TABLE k3l_cast_action_y2024m05 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-05-01') TO ('2024-06-01');

CREATE TABLE k3l_cast_action_y2024m06 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-06-01') TO ('2024-07-01');

CREATE TABLE k3l_cast_action_y2024m07 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-07-01') TO ('2024-08-01');

CREATE TABLE k3l_cast_action_y2024m08 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-08-01') TO ('2024-09-01');

CREATE TABLE k3l_cast_action_y2024m09 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-09-01') TO ('2024-10-01');

CREATE TABLE k3l_cast_action_y2024m10 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-10-01') TO ('2024-11-01');

CREATE TABLE k3l_cast_action_y2024m11 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-11-01') TO ('2024-12-01');

CREATE TABLE k3l_cast_action_y2024m12 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2024-12-01') TO ('2025-01-01');

CREATE TABLE k3l_cast_action_y2025m01 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_cast_action_y2025m02 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_cast_action_y2025m03 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_cast_action_y2025m04 PARTITION OF k3l_cast_action FOR
VALUES
FROM
    ('2025-04-01') TO ('2025-05-01');
...
CREATE INDEX k3l_cast_action_v1_covering_idx ON k3l_cast_action_v1 USING btree (cast_hash, action_ts, fid) INCLUDE (casted, replied, recasted, liked);
...
SELECT
,
    REFERENCES ON TABLE public.k3l_cast_action_v1 TO k3l_readonly;

CREATE TABLE k3l_cast_action_v1_y2024m09 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2024-09-01') TO ('2024-10-01');

CREATE TABLE k3l_cast_action_v1_y2024m10 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2024-10-01') TO ('2024-11-01');

CREATE TABLE k3l_cast_action_v1_y2024m11 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2024-11-01') TO ('2024-12-01');

CREATE TABLE k3l_cast_action_v1_y2024m12 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2024-12-01') TO ('2025-01-01');

CREATE TABLE k3l_cast_action_v1_y2025m01 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_cast_action_v1_y2025m02 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_cast_action_v1_y2025m03 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_cast_action_v1_y2025m04 PARTITION OF k3l_cast_action_v1 FOR
VALUES
FROM
    ('2025-04-01') TO ('2025-05-01');
...
SELECT
,
    REFERENCES ON VIEW public.k3l_cast_action TO k3l_readonly;
...
SELECT
    ...
    cfids.*
FROM
    k3l_channel_fids as cfids WITH NO DATA;
...
SELECT
,
    REFERENCES ON public.k3l_channel_rank TO k3l_readonly;
...
CREATE INDEX cura_hidden_fids_ch_fid_act_idx ON public.cura_hidden_fids USING btree (action, channel_id, hidden_fid, is_active);

GRANT
SELECT
,
    REFERENCES ON public.cura_hidden_fids TO k3l_readonly;
...
SELECT
,
    REFERENCES ON public.automod_data TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.warpcast_channels_data TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.k3l_top_casters TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.k3l_top_spammers TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.warpcast_followers TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.warpcast_members TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.verifications TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.links TO k3l_readonly;
...
CREATE
OR REPLACE FUNCTION end_week_9amoffset(timestamp, interval) RETURNS timestamptz AS $ $
SELECT
...
CREATE TABLE k3l_channel_metrics_y2025m01 PARTITION OF k3l_channel_metrics FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_channel_metrics_y2025m02 PARTITION OF k3l_channel_metrics FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_channel_metrics_y2025m03 PARTITION OF k3l_channel_metrics FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_channel_metrics_y2025m04 PARTITION OF k3l_channel_metrics FOR
VALUES
FROM
    ('2025-04-01') TO ('2025-05-01');
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_points_bal TO k3l_readonly;
...
CREATE TABLE k3l_channel_points_log_y2024m12 PARTITION OF k3l_channel_points_log FOR
VALUES
FROM
    ('2024-12-01') TO ('2025-01-01');

CREATE TABLE k3l_channel_points_log_y2025m01 PARTITION OF k3l_channel_points_log FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_channel_points_log_y2025m02 PARTITION OF k3l_channel_points_log FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_channel_points_log_y2025m03 PARTITION OF k3l_channel_points_log FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_channel_points_log_y2025m04 PARTITION OF k3l_channel_points_log FOR
VALUES
FROM
    ('2025-04-01') TO ('2025-05-01');
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_tokens_bal TO k3l_readonly;
...
CREATE TABLE public.k3l_channel_tokens_log (
    ...
    -- dist_id per channel group
    batch_id int2 NOT NULL,
    -- batch_id per dist_id
...
CREATE TABLE k3l_channel_tokens_log_y2024m12 PARTITION OF k3l_channel_tokens_log FOR
VALUES
FROM
    ('2024-12-01') TO ('2025-01-01');

CREATE TABLE k3l_channel_tokens_log_y2025m01 PARTITION OF k3l_channel_tokens_log FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_channel_tokens_log_y2025m02 PARTITION OF k3l_channel_tokens_log FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_channel_tokens_log_y2025m03 PARTITION OF k3l_channel_tokens_log FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');

CREATE TABLE k3l_channel_tokens_log_y2025m04 PARTITION OF k3l_channel_tokens_log FOR
VALUES
FROM
    ('2025-04-01') TO ('2025-05-01');
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_tokens_log TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_rewards_config TO k3l_readonly;
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_points_allowlist TO k3l_readonly;
...
CREATE TABLE public.k3l_channel_domains (
    ...
    -- constrain datatype to auto-fail on bad data
    domain int NOT NULL,
...
SELECT
,
    REFERENCES ON TABLE public.k3l_channel_openrank_req_ids TO k3l_readonly;
...
CREATE TABLE public.k3l_channel_openrank_results (
    ...
    rank bigint NOT NULL,
    insert_ts timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT k3l_channel_openrank_results_fkey FOREIGN KEY (channel_domain_id) REFERENCES public.k3l_channel_domains(id)
) PARTITION BY RANGE (insert_ts);
...
CREATE TABLE k3l_channel_openrank_results_y2024m11 PARTITION OF k3l_channel_openrank_results FOR
VALUES
FROM
    ('2024-11-01') TO ('2024-12-01');

CREATE TABLE k3l_channel_openrank_results_y2024m12 PARTITION OF k3l_channel_openrank_results FOR
VALUES
FROM
    ('2024-12-01') TO ('2025-01-01');

CREATE TABLE k3l_channel_openrank_results_y2025m01 PARTITION OF k3l_channel_openrank_results FOR
VALUES
FROM
    ('2025-01-01') TO ('2025-02-01');

CREATE TABLE k3l_channel_openrank_results_y2025m02 PARTITION OF k3l_channel_openrank_results FOR
VALUES
FROM
    ('2025-02-01') TO ('2025-03-01');

CREATE TABLE k3l_channel_openrank_results_y2025m03 PARTITION OF k3l_channel_openrank_results FOR
VALUES
FROM
    ('2025-03-01') TO ('2025-04-01');
