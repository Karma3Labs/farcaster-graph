...
# Query the DagRun
dag_run = (
    session.query(DagRun).filter(DagRun.dag_id == dag_id, DagRun.run_id == run_id).one()
)
...
for map_index in range(map_index_start, map_index_end + 1):
    try:
        ...
        print(
            f"Cleared task {task_id} with map index {map_index} for DAG {dag_id} and run ID {run_id}"
        )
...
session.commit()
print(
    f"Cleared tasks {task_id} with map indexes from {map_index_start} to {map_index_end} for DAG {dag_id} and run ID {run_id}"
)
