...
PGPASSWORD=$DB_PASSWORD \
$PSQL -e -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME \
  -c "DELETE FROM $DB_GLOBALTRUST WHERE date=(SELECT max(date) FROM $DB_TEMP_GLOBALTRUST); COMMIT;
INSERT INTO $DB_GLOBALTRUST SELECT * FROM $DB_TEMP_GLOBALTRUST; COMMIT;
