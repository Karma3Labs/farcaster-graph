...
logger.remove()
level_per_module = {"": "INFO", "app": settings.LOG_LEVEL, "silentlib": False}
logger.add(
    sys.stdout,
    colorize=True,
...
async def get_graph_neighbors(graph: igraph.Graph, fid: int, k: int, limit: int):
    try:
        ...
        neighbors = await run_in_threadpool(
            graph.neighborhood, vid, order=k, mode="out", mindist=k
        )
        if len(neighbors) == 0:
...
@app.get("/graph")
async def get_graph(
    fid: int, k: int, limit: int, graph: igraph.Graph = Depends(inject_graph)
):
    try:
...
async def reload_graph():
    try:
        ...
        g = await run_in_threadpool(
            igraph.Graph.Read_Pickle, settings.PERSONAL_IGRAPH_INPUT
        )
        app_state["graph"] = g
        return {"status": "ok"}
    except Exception as e:
