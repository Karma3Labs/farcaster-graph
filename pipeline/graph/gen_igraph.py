...
@Timer(name="main")
def main(incsv:Path,  outdir:Path, prefix:str, logger:logging.Logger, filtercsv:Path):
  ...
  if filtercsv is not None:
    ...
    with Timer(name="filter_i"):
      ...
      edges_df = pd.merge(edges_df, filter_df, left_on=['i'], how='left', right_on=[filter_col_name])
    ...
    with Timer(name="filter_j"):
      ...
      edges_df = pd.merge(edges_df, filter_df, left_on=['j'], how='left', right_on=[filter_col_name])
      edges_df = edges_df[edges_df[filter_col_name].notna()]
      edges_df = edges_df.drop(columns=[filter_col_name])
      logger.info(utils.df_info_to_string(edges_df, with_sample=True))
    utils.log_memusage(logger)
...
if __name__ == '__main__':
  ...
  parser.add_argument("-p", "--prefix",
                    ...
                    required=True,
                    type=str)
  parser.add_argument("-f", "--filtercsv",
  ...
  main(incsv=args.incsv, outdir=args.outdir, prefix=args.prefix, logger=logger, filtercsv=filtercsv)