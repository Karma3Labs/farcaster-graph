...
from .fetch_nodes_edges import fetch_fids_edges_from_csv
...
async def compute_task(
    ...
    try:
        ...
        while limit > 0 and degree <= 5:
            ...
            logger.trace(
                f"{process_label}FID {fid}: {degree}-degree neigbors scores: {k_scores}"
            )
            if len(k_scores) == 0:
                break
...
def compute_subprocess(
    ...
    logger.info(
        f"{process_label}sample of FIDs slice: {np.random.choice(slice_arr, size=min(5, len(slice)), replace=False)}"
    )
    results = [
        result
        for result in asyncio.run(
            compute_tasks_concurrently(
                maxneighbors, localtrust_df, slice_arr, process_label
            )
        )
    ]
    ...
    logger.info(
        f"{process_label} subprocess took {time.perf_counter() - subprocess_start} secs"
    )
    return slice_id


async def main(
    incsv: Path, outdir: Path, procs: int, chunksize: int, maxneighbors: int
):
    start_time = time.perf_counter()
    ...
    with ProcessPoolExecutor(max_workers=procs) as executor:
        tasks = [
            loop.run_in_executor(
                executor, compute_subprocess, outdir, maxneighbors, edges_df, slice
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--procs", help="number of processes to kick off", required=True, type=int
    )
    parser.add_argument(
        "-c",
        "--chunksize",
