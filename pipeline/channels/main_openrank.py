...
def fetch_results(
    ...
    req_ids_df = pd.read_csv(
        file, header=None, names=["channel_id", "interval_days", "domain", "req_id"]
    )
    ...
    for _, row in req_ids_df.iterrows():
        ...
        domain = row["domain"]
        req_id = row["req_id"]
        channel_domain_id = channel_domain_df[channel_domain_df["channel_id"] == cid][
            "id"
        ].values[0]

        toml_filename = _TOML_FILENAME_FORMAT.format(
            cid=cid, interval=interval, domain=domain
        )
        toml_file = os.path.join(out_dir, toml_filename)
        if not os.path.exists(toml_file):
            raise Exception(f"Missing toml_file {toml_file}")

        out_filename = _RANKING_FILENAME_FORMAT.format(
            cid=cid, interval=interval, domain=domain
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to download results for channel {cid}, interval {interval}, domain {domain}, req_id {req_id}: {e}"
            )
            continue
...
def process_domains(
    ...
    for cid in channel_ids_list:
        try:
            ...
            lt_filename = _LT_FILENAME_FORMAT.format(
                cid=cid, interval=interval, domain=domain
            )
            lt_file = os.path.join(out_dir, lt_filename)

            pt_filename = _PT_FILENAME_FORMAT.format(
                cid=cid, interval=interval, domain=domain
            )
            ...
            if (
                not os.path.exists(lt_file)
                or not os.path.exists(pt_file)
                or not os.path.exists(toml_file)
            ):
...
def write_openrank_files(
    ...
    max_result_size = len(
        set(localtrust_df["i"]) | set(localtrust_df["j"])
...
def append_delta_prev(
    ...
    if not (os.path.exists(prev_lt_file) and os.path.exists(prev_pt_file)):
        logger.warning(
            f"{prev_dir} is missing previous files for"
    ...
    logger.info(
        f"Previous localtrust: {utils.df_info_to_string(prev_lt_df, with_sample=True)}"
    )
    logger.info(
        f"Current localtrust: {utils.df_info_to_string(current_lt_df, with_sample=True)}"
    )
    prev_pt_df = pd.read_csv(prev_pt_file)
    logger.info(
        f"Previous pretrust: {utils.df_info_to_string(prev_lt_df, with_sample=True)}"
    )
    logger.info(
        f"Current pretrust: {utils.df_info_to_string(current_pt_df, with_sample=True)}"
    )
    ...
    if len(current_pt_df) == 0:
        logger.warning(
            f"Current pretrust is empty for channel {cid} with domain {domain} and interval {interval}"
        )
        prev_pt_df["v"] = 0.0
        return merged_lt_df, prev_pt_df
    if len(prev_pt_df) == 0:
        logger.warning(
            f"Previous pretrust is empty for channel {cid} with domain {domain} and interval {interval}"
        )
...
def gen_domain_files(
    ...
    for cid in channel_ids_list:
        try:
            ...
            if prev_dir:
                logger.info(
                    f"Processing previous files for channel {cid} with domain {domain} and interval {interval}"
                )
    ...
    if args.task == "fetch_domains":
        ...
        if args.task == "fetch_results":
            ...
            if args.task == "gen_domain_files":
                if not hasattr(args, "seed") or not hasattr(args, "prevdir"):
                    logger.error(
