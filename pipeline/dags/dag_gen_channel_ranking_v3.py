...
def create_dag():
    ...
    def tg_prep_db():
        sanitycheck_before_truncate = SQLCheckOperator(
            task_id="sanitycheck_before_truncate", sql=CHECK_QUERY, conn_id=_CONN_ID
        )
    ...
    def tg_compute():
        ...
        process_1d_tasks = process_channel_chunk.partial(interval=1).expand(
            chunk=extract_ids
        )
        process_7d_tasks = process_channel_chunk.partial(interval=7).expand(
            chunk=extract_ids
        )
        process_60d_tasks = process_channel_chunk.partial(interval=60).expand(
            chunk=extract_ids
        )
        process_lifetime_tasks = process_channel_chunk.partial(interval=0).expand(
            chunk=extract_ids
        )
        ...
        (
            ...
            >> process_7d_tasks
            >> process_60d_tasks
            >> process_lifetime_tasks
        )

    @task_group(group_id="refesh_db")
    def tg_db():
        sanitycheck_before_refresh = SQLCheckOperator(
            task_id="sanitycheck_before_refresh", sql=CHECK_QUERY, conn_id=_CONN_ID
        )
        ...
        #     trigger_dag_id="gen_channel_openrank",
