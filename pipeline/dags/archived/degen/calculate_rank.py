...
def load_localtrust_df_from_dune() -> pd.DataFrame:
    ...
    logger.info("Loaded localtrust values from Dune. Number of rows: {}", len(values_df))
    return values_df
...
def load_pretrust_df_from_localtrust(values_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pretrust_dict = {user: amount / total_received_sum for user, amount in total_received_amount.items()}
...
def run_eigentrust(localtrust_df: pd.DataFrame, pretrust_df: pd.DataFrame):
    localtrust = [l for l in localtrust_df[["i", "j", "v"]].to_dict("records") if l['v'] > 0.0]
    ...
    alpha = 0.1
    a = EigenTrust(host_url=settings.GO_EIGENTRUST_URL, alpha=alpha) #higher alpha weights pretrust more
