...
if [ -z "$TASK" ] || [ -z "$JOB_TYPE" ] || [ -z "$VENV" ]  || [ -z "$WORK_DIR" ]; then
  echo "Prep Usage: $0 -t prep -w [work_dir] -v [venv] -j [job_type] "
  echo "Fetch Usage:   $0 -t fetch -w [work_dir] -v [venv] -s [scope] -j [job_type] -c [csv_file] "
  echo "Daemon Usage:   $0 -t fetch -w [work_dir] -v [venv] -s [scope] -j [job_type] -c [csv_file] -d "
  echo "Cleanup Usage: $0 -t cleanup -w [work_dir] -v [venv] -j [job_type] "
  echo ""
  echo "Example: $0 -t prep -w . -v /home/ubuntu/farcaster-graph/publisher/.venv -j followers"
  echo "         $0 -t fetch-w . -v /home/ubuntu/farcaster-graph/publisher/.venv -c ./Top_Channels.csv -s top -j followers"
  echo "         $0 -t fetch-w . -v /home/ubuntu/farcaster-graph/publisher/.venv -c ./Top_Channels.csv -s all -j followers"
  echo "         $0 -t fetch -w . -v /home/ubuntu/farcaster-graph/publisher/.venv -c ./Top_Channels.csv -s all -d"
  echo "         $0 -t cleanup -w . -v /home/ubuntu/farcaster-graph/publisher/.venv -j followers"
  echo ""
  echo "Params:"
