...
if [ -z "$TASK" ] || [ -z "$JOB_TYPE" ] || [ -z "$VENV" ]  || [ -z "$WORK_DIR" ]; then
  ...
  echo ""
  echo "Example: $0 -o fetch -w . -v /home/ubuntu/farcaster-graph/publisher/.venv -c ./Top_Channels.csv -s top -j followers"
  ...
  echo ""
  echo "Params:"
  echo "  [task] The task to perform: fetch or cleanup"
...
if [ "$TASK" = "fetch" ]; then
  ...
  echo "Running channel followers cleanup with flags $JOB_TYPE"
