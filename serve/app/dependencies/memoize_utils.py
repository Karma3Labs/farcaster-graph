...
class EncodedMethodNameAndArgsExcludedKeyExtractor(KeyExtractor):
    ...
    def __init__(self, skip_first_arg_as_self=False, skip_args:list[int] = [], skip_kwargs: list[str] = []) -> None:
    ...
    def format_key(self, method_reference, call_args: Tuple[Any, ...], call_kwargs: Dict[str, Any]) -> str:
        if self._skip_args:
            call_args = [arg for i, arg in enumerate(call_args) if i not in self._skip_args]
        if self._skip_kwargs:
            call_kwargs = {k: v for k, v in call_kwargs.items() if k not in self._skip_kwargs}
